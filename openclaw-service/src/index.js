...
// State to track current persona, activity, and memory (Persisted via MongoDB)
    // State to track current persona, activity, and memory (Persisted via MongoDB)
    userPersonas = new PersistentMap(User, { mode: 'mongo', service: 'openclaw' });
    userActivity = new PersistentMap(User, { mode: 'mongo', service: 'openclaw' });
    userProfiles = new PersistentMap(User, { mode: 'mongo', service: 'openclaw' }); 
    userSubscriptions = new PersistentMap(User, { mode: 'mongo', service: 'openclaw' }); 
    userMessageHistory = new PersistentMap(Chat, { mode: 'mongo', service: 'openclaw' }); 
    userChatHistory = new PersistentMap(Chat, { mode: 'mongo', service: 'openclaw' }); 
    userMemories = new PersistentMap(Memory, { mode: 'mongo', service: 'openclaw' });
    anchorMemories = new VectorMemory(Memory, { mode: 'mongo', service: 'openclaw' });
    ...
    // Health Check
    router.get('/health', (req, res) => res.status(200).json({ status: 'healthy', service: 'openclaw', timestamp: new Date().toISOString() }));
    ...
    if (sharedApp) {
        sharedApp.use('/openclaw', router);
        log('API', 'OpenClaw Profile Sync mounted to /openclaw');
    } else {
