...
// Setup production-grade telemetry
const telemetry = new Telemetry('celeb');
...
// State to track current persona, activity, and memory
    ...
    userPersonas = new PersistentMap(User, { mode: 'mongo', service: 'celeb' });
    userActivity = new PersistentMap(User, { mode: 'mongo', service: 'celeb' });
    userProfiles = new PersistentMap(User, { mode: 'mongo', service: 'celeb' }); 
    userSubscriptions = new PersistentMap(User, { mode: 'mongo', service: 'celeb' }); 
    userMessageHistory = new PersistentMap(Chat, { mode: 'mongo', service: 'celeb' }); 
    userChatHistory = new PersistentMap(Chat, { mode: 'mongo', service: 'celeb' }); 
    userMemories = new PersistentMap(Memory, { mode: 'mongo', service: 'celeb' });
    anchorMemories = new VectorMemory(Memory, { mode: 'mongo', service: 'celeb' });
    ...
    // Health Check
    router.get('/health', (req, res) => res.status(200).json({ status: 'healthy', service: 'celeb', timestamp: new Date().toISOString() }));
    ...
    if (sharedApp) {
        sharedApp.use('/celeb', router);
        log('API', 'Celeb Profile Sync mounted to /celeb');
    } else {
        const port = process.env.PORT || 3009;
        const app = express();
        app.use('/celeb', router);
...
bot.on('message', async (msg) => {
    ...
    if (text.startsWith('/persona') || text.startsWith('/summon')) {
        ...
        if (subCommand === 'list') {
            safeSendMessage(chatId, "Available Celeb Personas:\n- `tay_vibe` (The Poetic Star)\n- `iron` (The Alpha Leader)\n- `elon_spark` (The Visionary)\n- `kendro-lamar` (The Deep Thinker)\n- `srk_charm` (The Charismatic King)\n\nTry `/persona <id>`!");
