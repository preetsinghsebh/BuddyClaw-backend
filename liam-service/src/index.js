...
const warnedUsers = new Set();
    ...
    userPersonas = new PersistentMap(User, { mode: 'mongo', service: 'liam' });
    userActivity = new PersistentMap(User, { mode: 'mongo', service: 'liam' });
    userProfiles = new PersistentMap(User, { mode: 'mongo', service: 'liam' }); 
    userSubscriptions = new PersistentMap(User, { mode: 'mongo', service: 'liam' }); 
    userMessageHistory = new PersistentMap(Chat, { mode: 'mongo', service: 'liam' }); 
    userChatHistory = new PersistentMap(Chat, { mode: 'mongo', service: 'liam' }); 
    userMemories = new PersistentMap(Memory, { mode: 'mongo', service: 'liam' });
    anchorMemories = new VectorMemory(Memory, { mode: 'mongo', service: 'liam' });
    ...
    // Health Check
    router.get('/health', (req, res) => res.status(200).json({ status: 'healthy', service: 'liam', timestamp: new Date().toISOString() }));
    ...
    if (sharedApp) {
        sharedApp.use('/liam', router);
        log('API', 'Liam Profile Sync mounted to /liam');
    } else {
