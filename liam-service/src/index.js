...
const telemetry = new Telemetry('liam');
...
const userPersonas = new PersistentMap(User, { mode: 'mongo', service: 'liam' });
const userActivity = new PersistentMap(User, { mode: 'mongo', service: 'liam' });
const userProfiles = new PersistentMap(User, { mode: 'mongo', service: 'liam' }); 
const userSubscriptions = new PersistentMap(User, { mode: 'mongo', service: 'liam' }); 
const pendingReplies = new Map(); // Not persisted (transient)
const userMessageHistory = new PersistentMap(Chat, { mode: 'mongo', service: 'liam' }); 
const userChatHistory = new PersistentMap(Chat, { mode: 'mongo', service: 'liam' }); 
const userMemories = new PersistentMap(Memory, { mode: 'mongo', service: 'liam' });
const anchorMemories = new VectorMemory(Memory, { mode: 'mongo', service: 'liam' });
...
app.get('/health', (req, res) => res.status(200).json({ status: 'healthy', service: 'liam', timestamp: new Date().toISOString() }));
...
app.listen(3007, () => log('API', 'Profile Sync Server listening on port 3007'));
...
bot.on('message', async (msg) => {
    ...
    if (text.startsWith('/start')) {
        ...
        if (startParam && startParam.startsWith('persona_')) {
            ...
            log(`TG-${chatId}`, `Pure /start detected. Triggering Liam greeting.`);
            const personaId = 'partner';
