...
const warnedUsers = new Set();
    ...
    userPersonas = new PersistentMap(User, { mode: 'mongo', service: 'safespace' });
    userActivity = new PersistentMap(User, { mode: 'mongo', service: 'safespace' });
    userProfiles = new PersistentMap(User, { mode: 'mongo', service: 'safespace' }); 
    userSubscriptions = new PersistentMap(User, { mode: 'mongo', service: 'safespace' }); 
    userMessageHistory = new PersistentMap(Chat, { mode: 'mongo', service: 'safespace' }); 
    userChatHistory = new PersistentMap(Chat, { mode: 'mongo', service: 'safespace' }); 
    userMemories = new PersistentMap(Memory, { mode: 'mongo', service: 'safespace' });
    anchorMemories = new VectorMemory(Memory, { mode: 'mongo', service: 'safespace' });
    ...
    // Health Check
    router.get('/health', (req, res) => res.status(200).json({ status: 'healthy', service: 'safespace', timestamp: new Date().toISOString() }));
    ...
    if (sharedApp) {
        sharedApp.use('/safespace', router);
        log('API', 'SafeSpace Profile Sync mounted to /safespace');
    } else {
        const port = process.env.PORT || 3010;
        const app = express();
        app.use('/safespace', router);
