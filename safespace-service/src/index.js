...
const telemetry = new Telemetry('safespace');
...
const userPersonas = new PersistentMap(User, { mode: 'mongo', service: 'safespace' });
const userActivity = new PersistentMap(User, { mode: 'mongo', service: 'safespace' });
const userProfiles = new PersistentMap(User, { mode: 'mongo', service: 'safespace' }); 
const userSubscriptions = new PersistentMap(User, { mode: 'mongo', service: 'safespace' }); 
const pendingReplies = new Map(); // Not persisted (transient)
const userMessageHistory = new PersistentMap(Chat, { mode: 'mongo', service: 'safespace' }); 
const userChatHistory = new PersistentMap(Chat, { mode: 'mongo', service: 'safespace' }); 
const userMemories = new PersistentMap(Memory, { mode: 'mongo', service: 'safespace' });
const anchorMemories = new VectorMemory(Memory, { mode: 'mongo', service: 'safespace' });
...
app.get('/health', (req, res) => res.status(200).json({ status: 'healthy', service: 'safespace', timestamp: new Date().toISOString() }));
...
app.get('/api/profile/:chatId', verifyInternalToken, (req, res) => {
    ...
    const personaId = userPersonas.get(chatId) || 'caring_mom';
    res.json({ ...profile, personaId });
});

app.listen(3010, () => log('API', 'Profile Sync Server listening on port 3010'));
...
bot.on('message', async (msg) => {
    ...
    if (text.startsWith('/persona') || text.startsWith('/summon')) {
        ...
        if (subCommand === 'list') {
            safeSendMessage(chatId, "Available Safe Space Personas:\n- `listener` (Deep Empathy)\n- `calm-guide` (Peaceful Support)\n- `bua` (The Toxic Relative)\n- `chill_chacha` (The Cool Uncle)\n- `late_night_dadi` (Caring Grandma)\n- `big_sister` (Strict but Caring)\n\nTry `/persona <id>`!");
